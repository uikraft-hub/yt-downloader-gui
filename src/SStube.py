...
class SSTubeGUI:
    def __init__(self, root):
        ...
        self.icons = {
            ...
            "settings": self.load_icon(
                os.path.join(self.base_dir, "assets", "settings.png")
            ),
        }
        ...
        try:
            light_img = Image.open(
                os.path.join(self.base_dir, "assets", "light.png")
            ).resize((100, 100), Image.LANCZOS)
            dark_img = Image.open(
                os.path.join(self.base_dir, "assets", "dark.png")
            ).resize((100, 100), Image.LANCZOS)
    ...
    def create_settings_frame(self):
        ...
        update_btn = ttk.Button(
            frame, text="Check for Updates", command=self.check_for_update
        )
        ...
        try:
            ...
            with zipfile.ZipFile(temp_zip_path, "r") as zip_ref:
                ...
                try:
                    ...
                    raise Exception(
                        "Permission denied. Please run as administrator or install to a writable folder."
                    )
            ...
            if latest_version > CURRENT_VERSION:
                ...
                if messagebox.askyesno("Update Available", msg):
                    webbrowser.open(
                        data.get(
                            "html_url",
                            "https://github.com/YourGitHubUserName/SSTube/releases",
                        )
                    )
            else:
    ...
    def delete_history(self):
        confirm = messagebox.askyesno(
            "Delete History", "Are you sure you want to delete the download history?"
        )
        if confirm:
            self.history = []
